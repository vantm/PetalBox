...
{
    public Aff<RT, Product> of(ProductId id) =>
        ...
        from parsed in SuccessEff(res.Match((rows) => rows.Length == 0 ? None : Some(Product.Restore(rows[0]))))
    ...
    public Aff<RT, Seq<Product>> all(SelectParams @params) =>
        ...
        from parsed in SuccessEff(res.Match(rows => rows.Select(Product.Restore).ToSeq()))
        from ret in flattenEff(parsed)
        select ret;
